...
--    - public_links (public, 5MB limit, image/png, image/jpeg, image/webp)
...
CREATE POLICY annotated_select ON storage.objects
  FOR SELECT USING (
...
CREATE POLICY public_links_select ON storage.objects
  FOR SELECT USING (bucket_id = 'public_links');
